...
| `scripts/reinforce/reinforce.py` | REINFORCE generation strategy - reward-weighted policy-gradient training on the full generated batch each step |
| `scripts/hill_climb/hill_climb.py` | Hill Climb generation strategy - trains only on the top-*k* highest-reward molecules per batch via unweighted maximum-likelihood loss |
| `scripts/reinforce/norl.py` | No-RL baseline - samples from the frozen pretrained prior with no training, for comparison against the two RL strategies |
...
Each run generates 10,112 molecules (matching the thesis's reported dataset sizes) and writes a CSV of SMILES + scores. `norl.py` writes to `norl.csv` by default — rename this or adjust `log_dir` to match whatever filename the analysis pipeline's `config.py` expects (see `chemical_analysis/README.md`).
...
- **`norl.py`** never instantiates a trainable policy — it loads the frozen pretrained prior once, wraps generation in `torch.no_grad()`, and never calls an optimiser step.
